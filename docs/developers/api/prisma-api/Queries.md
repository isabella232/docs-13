...
- *Object queries*: Fetch single or multiple nodes of a certain type
...
In general, the GraphCMS API is generated based on its data model.

In the following, we will explore example queries with this data model:
...
> Note that more aggregations will be added over time.
...
!!! note
    `likedPosts` is not part of the above mentioned data model but can easily be added by adding the corresponding field to the `User` type: 
    `likedPosts: [Post!]! @relation(name: "LikedPosts")`. 
    
    Note that we also provide a `name` for the relation to resolve the ambiguity we would otherwise create because there are two relation fields targetting `Post` on the `User` type.
...
!!! note
    `OR` and `AND` accept a _list_ as input where each list item is an object and therefore needs to be wrapped with `{}`, for example: `AND: [{title_in: ["My biggest Adventure", "My latest Hobbies"]}, {published: true}]`
...
Use the API Explorer to explore available filter conditions.
...
When querying all nodes of a specific type, you can supply arguments that allow you to _paginate_ the query response.
...
Note that *a maximum of 1000 nodes* can be returned per pagination field on the shared cluster.
